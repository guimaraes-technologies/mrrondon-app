...
namespace MrRondon.Extensions
{
    public class BindablePicker : Picker
    {
        ...
        {
            if (!(bindable is BindablePicker picker)) return;
            picker.Items.Clear();
            if (newvalue == null) return;
            foreach (var item in newvalue) picker.Items.Add(item.ToString());
        }

        private void OnSelectedIndexChanged(object sender, EventArgs eventArgs)
        {
            if (SelectedIndex < 0 || SelectedIndex > Items.Count - 1) SelectedItem = null;
